...
if (@sa < 3) {
    print "Statistic.wait_io: 100";
    print "Message.wait_io: ERROR: vmstat output too short.\n";
    exit 1;
}
...
if (defined $index) {
    my $wa = $data[$index];
    print "Statistic.wait_io: $wa\n";
    print "Message.wait_io: CPU wait IO in percentage: $wa\n";
    exit 0;
} else {
    print "Statistic.wait_io: 100";
    print "Message.wait_io: ERROR: Can't find CPU wait IO (wa) in output of -vmstat- command.\n";
    exit 1;
}
