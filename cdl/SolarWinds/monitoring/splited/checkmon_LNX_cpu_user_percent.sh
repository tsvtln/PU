...
if ! command -v sar >/dev/null; then
    ...
    echo "Message: Unable to find 'sar' command - please check and install it on the target server." | tr -d '\n' | paste -sd ''
    exit 1
fi
...
if [[ $cpu_user_percent -ge 90 ]]; then
    top_processes=$(ps -eo pcpu,comm --sort=-pcpu | \
        ...
        awk 'NR>1 { printf "#%d %s (CPU: %s%%); ", NR-1, $2, $1 }' | head -n 1)
...
# output
echo "Statistic:$cpu_user_percent"
echo "Message: $message" | tr -d '\n' | paste -sd ''
