...
class Uploader:
    ...
    def __init__(self, chunk_size: int = 1024 * 1024) -> None:
        ...
        self.mounter: Optional[AzureFileShareMounter] = None
        self.share_mounted: bool = False

    def mount_share(self, username: Optional[str] = None, password: Optional[str] = None) -> Tuple[bool, str]:
        """
    ...
    def upload(
        ...
        items = list((target, src) for target, src in files_map.items() if src)

        total_bytes = self._compute_total_size(items)
        bytes_done = 0

        # Create version-based directory for the NEW upload
        from bin.global_vars import GlobalVars
        version_dir = os.path.join(dest_dir, GlobalVars.uipath_version)

        if progress:
            progress(0, 100, f"Creating version folder: {GlobalVars.uipath_version}")
        ...
        for target_name, src_path in items:
            ...
            # Save the new file to the VERSION folder as UiPathRemoteRuntime.msi
            dst_path = os.path.join(version_dir, "UiPathRemoteRuntime.msi")

            if progress:
                progress(0, total_bytes, f"Uploading to {GlobalVars.uipath_version}\\UiPathRemoteRuntime.msi")
            ...
            with open(src_path, 'rb') as src, open(dst_path, 'wb') as dst:
                while True:
                    ...
                    if progress:
                        progress(bytes_done, total_bytes, f"{GlobalVars.uipath_version}\\UiPathRemoteRuntime.msi")
